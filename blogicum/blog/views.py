...
def post_detail(request, pk):
    ...
    if request.user != post.author:
        if (
            ...
            )  # Дата публикации позже текущего времени (если дата указана)
...
def add_comment(request, pk):
    ...
    )  # Рендерим обратно detail.html с формой и ошибками
...
class CommentUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    template_name = (
        "blog/comment_edit.html"  # Создадим простой шаблон для редактирования
    )
...
def edit_profile(request, username):
    ...
    if request.user != user_to_edit:
        return redirect(
            "blog:profile_detail", username=username
        )  # Перенаправляем, если не владелец
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect(
                "blog:profile_detail", username=username
            )  # Перенаправляем на страницу профиля после сохранения
    else:
